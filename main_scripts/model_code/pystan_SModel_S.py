...
Build Selective Model (SModel) using Pystan
Three parts of code
...
#   S model : creates model for posterior probability of (cell_number, selection coefficient) given barcode-count
#

pystan_modelcode_S_model_S = """
// ================================================
...
transformed parameters {
        ...
        real<lower=0>   theta_              =  (a_/k_) * exp( b_ *(selection_coefficient-mean_s)/sqrt(var_s));          // scale parameter of Gamma distribution
...
model {
        ...
        //  selectio coefficient is samplied from Normal distribution
        
        selection_coefficient ~ normal(mean_s, sqrt(var_s));
...
generated quantities {
        ...
        real log_joint_prob = normal_lpdf(selection_coefficient | mean_s, sqrt(var_s)) 
...
if __name__ == '__main__':
    ...
    input_data = { 'barcode_count': bc_count, 'read_depth': RD, 'population_size': N, 
                 'epsilon': epsilon, 'k_': k, 'a_': a_, 'b_': b_,  'meanfitness': meanfitness,
                 'cycle': cycle, 'dilution_ratio': dilution_ratio, 'mean_s': mean_s, 'var_s': var_s
        }
    ...
    fig.savefig(script_path+'/'+plot_file_name, dpi=800)
